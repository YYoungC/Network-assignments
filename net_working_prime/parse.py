...
class Echo(threading.Thread):
    ...
    def send(self, path, meth):       # if the requested dir is a file, invoke this function
        ...
        if meth == "GET":           # if the method is get
            ...
            self.conn.send(          # turn the response into bytes and write them
                stri.join(seq)
            )
    ...
    def sendir(self, path):           # if the requested dir is a dir, invoke this function
        status = "HTTP/1.0 200 OK"          # the following are the response
    ...
    def error(self, sta):             # if error occurs, invoke this function
        status = ""
        ...
        self.conn.send(      # write the response
            stri.join(seq)
        )
